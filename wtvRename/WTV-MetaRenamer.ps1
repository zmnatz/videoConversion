...
# 0.23     Improvement to skip files if protected (work item 2542)
#
...
function GetSkipIfProtected()
{
   # Define the default result
   # Default to $false - we don't skip the file if it is protected; this was the behaviour before
   # the protected check code was added therefore behaviour is unchanged by default.
   $result = $false
   
   try {
      # Only override the default value if a value has actually been provided!
      if ($my_config.config.skip_if_protected -ne "")
         { $result = SafeBooleanConvert $my_config.config.skip_if_protected }
   }
   
   catch {
      Write-Verbose "... error while retrieving skip_if_protected element: $($_.Exception.Message)"
   }
...
function GetAttributeProtected()
{
   # Define the default result
   $result = "Protected"
   ...
   try {
      # Only override the default value if a value has actually been provided!
      if ($my_config.config.attribute_protected -ne "")
         { $result = $my_config.config.attribute_protected }
   }
   
   catch {
      Write-Verbose "... error while retrieving attribute_protected element: $($_.Exception.Message)"
   }
...
function ProcessFile($filename)
{
    ...
    # V0.23 - skip protected files (work item 2542)
    if ($index_protected -ne -1)
    {
        $is_protected = $($folder.GetDetailsOf($file, $index_protected))
        if ($is_protected -eq "Yes")
        {
            if ($skip_if_protected -eq $true)
            {
                $process_file = $false
                Write-HostAndLog "... file is protected and 'skip if protected' is true; skipping"
            }
            else
            {
                Write-HostAndLog "... file is protected but 'skip if protected' is not true"
            }
        }
    }
...
$skip_if_protected = GetSkipIfProtected
$min_age = GetMinAge
...
$attribute_protected = GetAttributeProtected
...
foreach ($index in 0..300)
{
    ...
    if ($($folder.GetDetailsOf($null, $index)) -eq $attribute_protected)
    {
        Write-VerboseAndLog "... found '$attribute_protected' at index $index"
        $index_protected = $index
    }
}
